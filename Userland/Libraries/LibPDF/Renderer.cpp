...
PDFErrorOr<NonnullRefPtr<Gfx::Bitmap>> Renderer::make_mask_bitmap_from_array(NonnullRefPtr<ArrayObject> array, NonnullRefPtr<StreamObject> image)
{
    ...
    size_t bytes_per_line = ceil_div(width * bits_per_component * n_components, 8ull);
    if (bytes_per_line * height > content.size())
        return Error(Error::Type::MalformedPDF, "Image data incomplete for mask array processing");

    auto mask = MUST(Gfx::Bitmap::create(Gfx::BitmapFormat::BGRA8888, { width, height }));

    if (bits_per_component == 16) {
        auto const* data = reinterpret_cast<BigEndian<u16> const*>(content.data());
        for (int y = 0; y < height; ++y) {
            for (int x = 0; x < width; ++x) {
                bool is_masked = true;
                for (size_t i = 0; i < n_components; ++i) {
                    u16 sample = data[y * (width * n_components) + x * n_components + i];
                    if (sample < min[i] || sample > max[i]) {
                        is_masked = false;
                        break;
                    }
                }
                mask->set_pixel(x, y, Color::from_argb(is_masked ? 0x00000000 : 0xffffffff));
            }
        }
    } else {
        ...
        for (int y = 0; y < height; ++y) {
            for (int x = 0; x < width; ++x) {
                bool is_masked = true;
                for (size_t i = 0; i < n_components; ++i) {
                    size_t x_index = (x * n_components + i) * bits_per_component;
                    size_t byte_index = x_index / 8;
                    size_t bit_index = x_index % 8;
                    u8 sample = (data[y * bytes_per_line + byte_index] >> (8 - bits_per_component - bit_index)) & ((1 << bits_per_component) - 1);
                    if (sample < min[i] || sample > max[i]) {
                        is_masked = false;
                        break;
                    }
