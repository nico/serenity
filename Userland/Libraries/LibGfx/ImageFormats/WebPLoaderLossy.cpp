...
ErrorOr<VP8Header> decode_webp_chunk_VP8_header(ReadonlyBytes vp8_data)
{
    ...
    if (!show_frame)
        return Error::from_string_literal("WebPImageDecoderPlugin: 'VP8 ' chunk has invalid visibility for webp iamge");
            ...
            for (int mb_x = 0; mb_x < macroblock_width; ++mb_x, ++macroblock_index) {
                ...
                for (int i = have_y2 ? 0 : 1; i < 25; ++i) {
                    ...
                    if (metadata.skip_coefficients) {
                        //continue;
                        return Error::from_string_literal("skip_coefficients");
                        goto clear_flags;
                    }
