...
Generator<ProgressionData, Empty> ResolutionLevelPositionComponentLayerProgressionIterator::generator()
{
    ...
    //              for each l = 0,..., L – 1
    //                  packet for component i, resolution level r, layer l, and precinct k."
    // XXX motivation is to walk precincts in different resolution layers at around the same time,
    // even if different resolution layers have different precinct counts.
    for (int r = 0; r <= m_max_number_of_decomposition_levels; ++r) {
