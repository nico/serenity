            ...
            for (int x = tx0; x < tx1; ++x) {
                for (int i = 0; i < C; ++i) {
                    ...
                    if ((y % (YRsizs[i] * (1 << (PPys[r * C + i] + N_Ls[i] - r))) == 0)
                        || ((y == ty0) && (try0 * (1 << (N_Ls[i] - r)) % (1 << (PPys[r * C + i] + N_Ls[i] - r)) != 0))) {
                        if ((x % (XRsizs[i] * (1 << (PPxs[r * C + i] + N_Ls[i] - r))) == 0)
                            || ((x == tx0) && (trx0 * (1 << (N_Ls[i] - r)) % (1 << (PPxs[r * C + i] + N_Ls[i] - r)) != 0))) {
                            if (int k = compute_precinct(x, y, r, i); k < precinct_counts[r * C + i]) {
                                for (int l = 0; l < m_layer_count; ++l) {
                                    co_yield ProgressionData { l, r, i, k };
                                }
                            }
