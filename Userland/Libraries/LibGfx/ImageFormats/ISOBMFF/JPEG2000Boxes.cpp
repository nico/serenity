...
ErrorOr<void> JPEG2000PaletteBox::read_from_stream(BoxStream& stream)
{
    ...
    for (u16 i = 0; i < number_of_entries; ++i) {
        ...
        for (auto const& bit_depth : bit_depths) {
            ...
            if (bit_depth.is_signed && (value & (1 << (bit_depth.depth - 1))))
                value |= ~((1 << bit_depth.depth) - 1);
...
{
    Box::dump(prepend);
    outln("{}- number_of_entries = {}", prepend, bit_depths.size());
    outln("{}- number_of_palette_columns = {}", prepend, palette_entries.size());
